...
async function main() {
  ...
  const target = sbxs.find((s: any) => s.name?.startsWith("dump-")) ?? sbxs[0];
  console.log("target:", target?.name, "session:", target?.currentSessionId);
  const sessionId = target?.currentSessionId;
  try {
    const intr: any = await client.openInteractive({ sessionId, projectId });
    console.log("=== INTERACTIVE RESPONSE (full) ===");
    console.log(JSON.stringify(intr.json ?? intr, null, 1));
  } catch (e) {
    console.log("interactive ERR:", (e as Error).message.slice(0, 400));
  }
}
